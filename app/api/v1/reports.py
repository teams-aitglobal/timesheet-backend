...
from app.services.reports import csv_utils, project_hours_report_service, timesheet_report_service
...
async def get_timesheet_report(
    ...
    if is_csv:
        return csv_utils.rows_to_csv_response(rows, filename="timesheet_report.csv")
...
async def get_project_hours_report(
    ...
    format: Literal["json", "csv"] = Query(default="json"),
    current_user: User = Depends(require_role(SUPER_ADMIN, PROGRAM_MANAGER)),
    db: AsyncSession = Depends(get_db),
    ...
    if format == "csv":
        return csv_utils.rows_to_csv_response(rows, filename="project_hours_report.csv")
    return rows
