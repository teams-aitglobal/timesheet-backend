...
def _assert_work_date_within_window(work_date: date) -> None:
    today = datetime.now(timezone.utc).date()
    if work_date > today:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="work_date cannot be in the future.")
    if work_date < today - timedelta(days=_MAX_BACKDATE_DAYS):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"work_date cannot be more than {_MAX_BACKDATE_DAYS} days in the past.",
        )


async def create_timesheet(db: AsyncSession, data: TimesheetCreate, actor: User) -> Timesheet:
    _assert_work_date_within_window(data.work_date)
